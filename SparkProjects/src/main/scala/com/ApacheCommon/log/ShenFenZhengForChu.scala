...
object ChuChuApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(spark.sql("select * from people_dataframe " +
      "where _c8 like '%南京%' and _c7>19800101 and _c6 = 'F' ").count())
//    spark.sql("select * from people_dataframe where _c8 like '%南京%' and _c7>19800101 ").show(false)

    val nanjing = spark.sql("select _c2 from people_dataframe" +
      " where _c8 like '%南京%' and _c7>19800101 and _c6 = 'F' ")
    println(nanjing.count())
    nanjing.show()

    nanjing.coalesce(1).write.format("csv").mode(SaveMode.Overwrite)
      .save("/Users/chandler/Desktop/chuchu/1-200")
